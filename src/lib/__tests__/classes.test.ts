...
test('модификаторов у объекта может быть несколько', t => {
  ...
  t.is(result, 'one one_two one_three');
});
...
test('модификатор не добавляется к элементу, если его значение — пустой объект', t => {
  const result = classes({
    one: {
      elem: {
        two: {
...
test('модификаторов для элемента может быть несколько', t => {
  ...
  t.is(result, 'one__two one__two_three one__two_four');
});
...
test('дублирует множесто модификаторов с независимого элемента на отобъектный элемент', t => {
  ...
  t.is(result, 'one__two one__two_three one__two_four two two_three two_four');
});
